 ...
 * @since 1.0.0
 */
public class MyJaxp13XPathTemplate extends AbstractXPathTemplate {
    ...
    public MyJaxp13XPathTemplate() {
        this(XPathFactory.DEFAULT_OBJECT_MODEL_URI);
    }

    public MyJaxp13XPathTemplate(String xpathFactoryUri) {
        try {
    ...
    private Object evaluate(String expression, Source context, QName returnType) throws XPathException {
        ...
        try {
            ...
            Exception localEx = new XPathException("Could not evaluate XPath expression [" + expression + "]", ex);
            log.error("parse exception", localEx);
        }
        catch (TransformerException ex) {
            // throw new XPathException("Could not transform context to DOM Node", ex);
            Exception localEx = new XPathException("Could not transform context to DOM Node", ex);
            log.error("parse exception", localEx);
        }
        catch (Exception ex) {
            // throw new XPathException(ex.getMessage(), ex);
            Exception localEx = new XPathException(ex.getMessage(), ex);
            log.error("parse exception", localEx);
        }
